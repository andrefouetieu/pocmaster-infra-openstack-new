...
Les stacks **01_vps** et **02_cluster** sont **séparés** (états Terraform différents). Les numéros 05 et 06 ne sont pas dans le même répertoire : ils ne se “croisent” pas.
...
Il n’y a **pas de 06** dans 01_vps.

**Si tu retires 05_ansible.tf du stack 01_vps :**
...
- **05_ansible.tf** n’existe que dans **01_vps** → il ne concerne que le VPN.
- **06_ansible_k3s.tf** n’existe que dans **02_cluster** → il ne concerne que le cluster K3s.

Tu peux avoir les deux dans le dépôt sans conflit : un `terraform apply` dans 01_vps exécute seulement le 05 ; un `terraform apply` dans 02_cluster exécute seulement le 06.
...
|-------|---------|------|-------------------|
| **01_vps** | **05_ansible.tf** | Lancer Ansible VPN (serveur + clients) après création de la VM. | VM VPN créée, mais pas d’OpenVPN installé ni de `.ovpn` générés. |
