...
Tu peux donc passer le provider en **~> 3.0.0** (ou 3.4.0) dans `01_vpn` et `02_cluster` sans modifier d’autres ressources.
...
4. **Faire la mise à jour** d’abord dans **01_vpn** ou **02_cluster** (un seul stack), vérifier `plan` et `apply`, puis appliquer à l’autre stack.
